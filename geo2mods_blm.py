...
#this script is a variant of the general geo2mods.py that is tailored specifically to BLM datasets. It takes a shapefile and transforms it into a MODS record
#syntax python scriptName nameOfShapefile.xml
...
def makeMods(doc):
    ...
    pub.set('authority','lcnaf')
    pub.set('valueURI','http://id.loc.gov/authorities/names/n80125846')
    pubTerm = SubElement(pub, 'mods:namePart')
    pubTerm.text = 'U.S. Department of the Interior, Bureau of Land Management (BLM)'
    ...
    creator.set('authority','lcnaf')
    creator.set('valueURI','http://id.loc.gov/authorities/names/n80125846')
    creatorTerm = SubElement(creator, 'mods:namePart')
    try:
        # creatorTerm.text = doc['creator']
        creatorTerm.text = 'U.S. Department of the Interior, Bureau of Land Management (BLM)'
    except KeyError:
        pass
    ...
    placeTerm = SubElement(subject, 'mods:geographic')
    subject.set('authority', 'fast')
    subject.set('valueURI','http://id.worldcat.org/fast/1210251')
    placeTerm.text = 'Colorado'
    ...
    form.set('authorityURI','https://id.loc.gov/authorities/genreForms/gf2011026297')
    ...
    filename = identifier.text.replace('.shp','')
    ...
    accessCond.text = 'The organization that has made the Item available believes that the Item is in the Public Domain under the laws of the United States, but a determination was not made as to its copyright status under the copyright laws of other countries. The Item may not be in the Public Domain under the laws of other countries. Please refer to the organization that has made the Item available for more information.'
    accessCond.set('xlink:href','https://rightsstatements.org/page/NoC-US/1.0/?language=en')
...
def main():
    ...
    filenameSection = re.sub("^.*\/", "", infile_path)
    filenameSection = filenameSection.replace('.shp','')
    print(filenameSection)
    doc=getGeoMetadata(infile_path)
    newtree=makeMods(doc)
    outfile_path = 'mods_'+filenameSection
